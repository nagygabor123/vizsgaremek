...
export default async function handler(req, res) {
  ...
  const query = `
      ...
      JOIN (
          SELECT 
            student_id,
            TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(s.class, ',', n.n), ',', -1)) AS class_group
          FROM 
            students s
          JOIN (
            SELECT 1 AS n UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4 UNION ALL SELECT 5
          ) n
          ON CHAR_LENGTH(s.class) - CHAR_LENGTH(REPLACE(s.class, ',', '')) >= n.n - 1
      ...
      WHERE 
        t.day_of_week = LOWER(DAYNAME(CURDATE()))
      GROUP BY 
